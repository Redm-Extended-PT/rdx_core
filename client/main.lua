...
Citizen.CreateThread(function()
	while true do
		Wait(5)
		local playerPed = PlayerPedId()
		if IsAppActive(`MAP`) ~= 0 and not Config.MapAnim  then
			SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
			Config.MapAnim  = true
			RDX.Animation(playerPed, "mech_inspection@mini_map@satchel", "enter")
			Wait(2000)
        ...
        if IsAppActive(`MAP`) ~= 1 and Config.MapAnim  then
		Config.MapAnim  = false
        RDX.Animation(playerPed, "mech_inspection@two_fold_map@satchel", "exit_satchel")
