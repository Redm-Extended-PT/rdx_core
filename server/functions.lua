RDX.Trace = function(msg)
	if Config.EnableDebug then
		print(('[redm_extended] [^2VESTIGIO^7] %s^7'):format(msg))
	end
end
...
RDX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if RDX.RegisteredCommands[name] then
		print(('[redm_extended] [^3AVISO^7] Um comando "%s" já está registrado, substituindo o comando'):format(name))
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[redm_extended] [^3AVISO^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[redm_extended] [^3AVISO^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('[redm_extended] [^3AVISO^7] %s^7'):format(msg))
					else
...
RDX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if RDX.ServerCallbacks[name] then
		...
		print(('[redm_extended] [^3AVISO^7] O retorno de chamada do servidor "%s" não existe. Certifique-se de que o arquivo do lado do servidor realmente está carregando, um erro nesse arquivo pode fazer com que ele não carregue.'):format(name))
	end
end
...
RDX.SavePlayer = function(xPlayer, cb)
	...
	asyncPool.startParallelAsync(function(results)
		print(('[redm_extended] [^2INFO^7] Jogador(es) salvo "%s^7"'):format(xPlayer.getName()))
...
RDX.SavePlayers = function(cb)
	...
	asyncPool.startParallelLimitAsync(8, function(results)
		print(('[redm_extended] [^2INFO^7] Jogador(es) %s salvo'):format(#xPlayers))
