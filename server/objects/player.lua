...
RDX.Player.Initialize = function(playerId, identifier, userData, cb)
	Citizen.CreateThread(function()
		...
		if (RDX.Players[u] ~= nil) then
			print(('[redm_extended] [^3ERRO^7] Tentando inicializar um player existente (%s)'):format(u))
		...
		RDX.Players[u].setJob = function(job, grade)
			...
			if RDX.DoesJobExist(job, grade) then
				...
				print(('[redm_extended] [^3AVISO^7] Ignorando o uso inválido de .setJob () para "%s"'):format(RDX.Players[u].identifier))
			end
		end
